...
public interface PayrollSummeryRepository extends JpaRepository<PayrollSummery, Integer > {
    ...
    @Query(nativeQuery = true,
            value = "SELECT p.id, p.name_in_full, IFNULL(d.dept_name,'NOT FOUND'), IFNULL(adi.department_name,'NOT FOUND') as Admin_Department_Id,\n" +
                    ...
                    "left join (select lm.emp_id, sum(lm.total_leave) as tot from leave_manager lm WHERE lm.leave_type in ('Casual','Casual - Sudden Illness','Casual - Funeral', 'Casual - Accident or vehicle breakdown') and lm.status = 5 and lm.from_date >= '2023-06-01' and lm.to_date < '2023-07-01' GROUP BY lm.emp_id) cas on cas.emp_id = p.id \n" +
                    "left join (select lm.emp_id, sum(lm.total_leave) as tot from leave_manager lm WHERE lm.leave_type in ('Annual') and lm.status = 5 and lm.from_date >= '2023-06-01' and lm.to_date < '2023-07-01' GROUP BY lm.emp_id) anu on anu.emp_id = p.id \n" +
                    "left join (select lm.emp_id, sum(lm.total_leave) as tot from leave_manager lm WHERE lm.leave_type in ('Day Off - May') and lm.status = 5 and lm.from_date >= '2023-06-01' and lm.to_date < '2023-07-01' GROUP BY lm.emp_id) day_off on day_off.emp_id = p.id \n" +
                    "left join (select lm.emp_id, sum(lm.total_leave) as tot from leave_manager lm WHERE lm.leave_type in ('Special') and lm.status = 5 and lm.from_date >= '2023-06-01' and lm.to_date < '2023-07-01' GROUP BY lm.emp_id) spe on spe.emp_id = p.id \n" +
                    "left join (select lm.emp_id, sum(lm.total_leave) as tot from leave_manager lm WHERE lm.leave_type in ('Probation Half Day') and lm.status = 5 and lm.from_date >= '2023-06-01' and lm.to_date < '2023-07-01' GROUP BY lm.emp_id) probation_half_day on probation_half_day.emp_id = p.id \n" +
                    "left join (select lm.emp_id, sum(lm.total_leave) as tot from leave_manager lm WHERE lm.leave_type in ('Short Leave') and lm.status = 5 and lm.from_date >= '2023-06-01' and lm.to_date < '2023-07-01' GROUP BY lm.emp_id) short_leave on short_leave.emp_id = p.id\n" +
                    "left join (select at.emp_id, count(at.id) as count from attendance at where at.date >= '2023-06-01' and at.date < '2023-07-01' and at.pay_roll_status = 8 group by at.emp_id) no_pay_weekday on no_pay_weekday.emp_id = p.id\n" +
                    "left join (select at.emp_id, count(at.id) as count from attendance at where at.date >= '2023-06-01' and at.date < '2023-07-01' and at.pay_roll_status = -1 group by at.emp_id) no_pay_weekend on no_pay_weekend.emp_id = p.id\n" +
                    "\n" +
    ...
    @Query(nativeQuery = true,
            value = "SELECT p.id, p.name_in_full, IFNULL(d.dept_name,'NOT FOUND'), IFNULL(adi.department_name,'NOT FOUND') as Admin_Department_Id,\n" +
                    ...
                    "where at.date >= '2023-06-01' and at.date < '2023-07-01' and \n" +
    ...
    @Query(nativeQuery = true,
            value = "select pe.id, pe.name_in_full, IFNULL(d.dept_name,'NOT FOUND'), IFNULL(adi.department_name,'NOT FOUND') as Admin_Department_Id,\n" +
                    ...
                    "\n" +
                    "left join (SELECT tl.user_id, IFNULL(SUM(IFNULL(tl.estimate,0)),0) as original_count FROM task_list tl WHERE tl.status = 5 AND tl.start_date >= '2023-06-01' AND tl.start_date < '2023-07-01' GROUP BY tl.user_id) obj3 on obj3.user_id = pe.id;")
